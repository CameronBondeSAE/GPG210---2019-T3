...
namespace Students.Luca.Scripts.Archive
{
    public class Thruster : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Physics.Raycast(transform.position, transform.up * -1, out hit, zeroForceHeight)
            ) // TODO do raycast from bottom/exhaust
            {
        ...
        void HandleTiltInput()
        {
            ...
            // Handle Tilt Left
            if (inputTiltLeft && ((localZRot - tiltAmount + 360) % 360) >= 360 - maxTiltAngle &&
                (localZRot - tiltAmount <= 360 || localZRot - tiltAmount <= 0))
            {
                tiltChange.z -= tiltAmount;
            }
            ...
            if (inputTiltBackward && ((localXRot - tiltAmount + 360) % 360) >= 360 - maxTiltAngle &&
                (localXRot - tiltAmount <= 360 || localXRot - tiltAmount <= 0))
            {
                tiltChange.x -= tiltAmount;
            }
