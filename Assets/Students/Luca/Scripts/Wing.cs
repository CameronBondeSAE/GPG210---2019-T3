...
public class Wing : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            transform.localRotation = Quaternion.RotateTowards(transform.localRotation, maxForwardRotation, rotationSpeed * Time.deltaTime);
        }else if (Input.GetKey(backwardRotateKey))
        {
            transform.localRotation = Quaternion.RotateTowards(transform.localRotation, maxBackwardRotation, rotationSpeed * Time.deltaTime);
        }
    ...
    private void ApplyAirForces()
    {
        ...
        Debug.DrawRay(transform.position,masterRb.transform.TransformDirection(localVelocity * angleToVelocityMultiplier * airForce * wingSizeMultiplier), Color.green);
        Debug.DrawRay(transform.position,masterRb.transform.TransformDirection(-localVelocity * angleToVelocityMultiplier * airForce * wingSizeMultiplier), Color.white);
        
        Vector3 finalForce = masterRb.transform.TransformDirection(-localVelocity * angleToVelocityMultiplier * airForce * wingSizeMultiplier);
        
        angleForceToGround = Vector3.Angle(Vector3.forward, localVelocity); // TODO HERE
